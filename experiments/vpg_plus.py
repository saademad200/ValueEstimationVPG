# VPG+ (VPG with PPO-style Clipping)
# This experiment combines the paper's insight (more value steps) with PPO-style clipping
# Hypothesis: VPG+ will achieve better stability and performance than either method alone
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__VPG_plus_clip{args.clip_eps}_vs{args.num_value_step}_s{args.seed}"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Normalize advantages (like PPO)
        if args.norm_adv:
            b_advantages = (b_advantages - b_advantages.mean()) / (b_advantages.std() + 1e-8)

        # Value loss with multiple steps (from paper)
